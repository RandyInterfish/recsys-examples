...
**`PASS(target_reached)` requires a measured profiler-off gain.** Grant `target_reached` only when the round's `verification.json` has a `perf` block with `clears_spread: true`. If there is no `verification.json` / no profiler-off `perf`, or `clears_spread: false`, you may **not** call it `target_reached` — attribution evidence alone (a sync row gone from the nsys trace) is not a gain. Such a round is at most `keep` with the gain recorded as unproven (use `blocked` / `diminishing_returns`), never `target_reached`.
